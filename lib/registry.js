...
Registry.prototype.destroy = function () {
  this._services.forEach(function (service) {
    debug('Registry destroyed %j', service)
    service._destroyed = true
  })
}

function start (registry, service) {
  if (service._activated) return
  service._activated = true

  debug('Registry start service %j %j', service, registry)
...
function stop (registry, service, cb) {
  ...
  debug('Registry stop service %j %j', service, registry)
...
function probe (mdns, service, cb) {
  ...
  function send () {
    ...
    mdns.query(service.fqdn, 'ANY', function () {
      debug('Registry probe query sent %s', service.fqdn)
  ...
  function done (exists) {
    debug('Registry probe done %j', !!exists)
  ...
  ;(function broadcast () {
    ...
    server.mdns.respond(packet, function () {
      ...
      if (!service.published) {
        debug('Registry service published %j', service)
        service._activated = true
...
function teardown (server, services, cb) {
  ...
  var records = flatten(services.map(function (service) {
  ...
  debug('Registry teardown %j', records)
